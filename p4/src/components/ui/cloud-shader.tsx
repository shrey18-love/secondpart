...
export const CloudShader = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Cap DPR at 1 for cloud shader to avoid rendering tens of millions of FBM noise pixels per frame on Retina displays
      const dpr = 1;
    ...
    const intersectionObserver = new IntersectionObserver(
      ([entry]) => {
        const WAS_VISIBLE = isVisible;
        isVisible = entry.isIntersecting;
        if (isVisible && !WAS_VISIBLE && running) {
